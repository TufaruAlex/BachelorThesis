			...
			IEnumerator InternalGenerate () {
				...
				for (var i = 0; i < world.prefabs.Length; i++) {
					...
					if (pref.singleFixed) {
						...
						var ob = GameObject.Instantiate(pref.prefab, p, Quaternion.identity) as GameObject;
						...
						for (var sx = 0; sx < world.subTiles; sx++) {
							for (var sz = 0; sz < world.subTiles; sz++) {
								...
								for (var j = 0; j < count; j++) {
									...
									var ob = GameObject.Instantiate(pref.prefab, p, RandomYRot(pref)) as GameObject;
									ob.transform.parent = root;
									//ob.SetActive ( false );
									//objs.Add ( ob );
									counter++;

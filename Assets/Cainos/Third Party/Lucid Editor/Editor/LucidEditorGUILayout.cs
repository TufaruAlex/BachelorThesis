...
namespace Cainos.LucidEditor
{
    public static class LucidEditorGUILayout
    {
        ...
        {
            var position = EditorGUILayout.GetControlRect(false, EditorGUIUtility.singleLineHeight + 5f, options);
            var value = LucidEditorGUI.SectionFoldout(position, foldout, label, backgroundColor, lineColor, toggleOnLabelClick);
            Space(3);
            return value;
        }
        ...
        public static int BeginTabGroup(int selected, GUIStyle style, string[] items, params GUILayoutOption[] options)
        {
            ...
            var position = EditorGUILayout.GetControlRect(false, EditorGUIUtility.singleLineHeight + 2f, options.Append(GUILayout.MinWidth(0f)).ToArray());
            position.xMin -= 4.2f;
            position.xMax += 4.2f;
            position.y -= 4.2f;
